...
typedef std::tr1::unordered_map<Symbol,Class_> ClassMap;
typedef std::tr1::unordered_map<Symbol,Feature> MethodMap;
typedef std::tr1::unordered_set<Symbol> SymbolSet;
typedef SymbolTable<Symbol,Symbol> SymbolTbl;

//Exceptions for Semantic Analysis Stage

struct SemantException : public std::runtime_error {
    SemantException(const std::string& s) : std::runtime_error(s) {}
    SemantException(const std::stringstream& ss) : std::runtime_error(ss.str()) {std::cerr << ss.str();}
};
...
struct Environment {
    ...
    /**
     * Adds entry to objectTbl.
     * First symbol is object ID, and second symbol is its type ID
     * Throws SemantException on invalid operations
     */
...
public:
   ...
   virtual void analyze() = 0;
   virtual void setEnvironment(Environment&) = 0;
...
public:
   ...
   void setEnvironment();
   void analyze(); //< perform semantic analysis on this class
...
public:
   ...
   void dump(ostream& stream, int n);
   void analyze(); //< perform semantic analysis on this method
...
public:
   ...
   Feature copy_Feature();
   void dump(ostream& stream, int n);
   void analyze(); //< perform semantic analysis on this attr
   void setEnvironment(Environment&);
...
public:
   ...
   Symbol getName() {return type_decl;}
   void analyze(); //< perform semantic analysis on this formal
...
public:
   ...
   Case copy_Case();
   void dump(ostream& stream, int n);
   void analyze(); //< perform semantic analysis on this branch
...
public:
   ...
   void dump(ostream& stream, int n);
   void analyze(); //< perform semantic analysis on this expr
